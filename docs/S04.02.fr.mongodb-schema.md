...
-- If there's an error, you would use:
-- ROLLBACK;
```

avec PL/pgSQL et une stored procedure

```SQL
-- Create a stored procedure to handle the transaction
...
AS $$
BEGIN
    -- Start the transaction implicitly (within the procedure)
    -- No explicit BEGIN statement is needed here.
    ...
    INSERT INTO transactions (account_id_from, account_id_to, amount, transaction_date)
    VALUES (account_id_from, account_id_to, amount, NOW());

EXCEPTION
    WHEN OTHERS THEN  -- Catch any exception
        -- Rollback the transaction
        RAISE EXCEPTION 'Transaction failed. Rolling back. Error: %', SQLERRM;
        ROLLBACK;
END;
$$;

-- Example usage: Call the stored procedure to transfer funds
...
1. **`conn = new Mongo();` and `db = conn.getDB("mydatabase");`**: Connects to the MongoDB server and selects the database.

2. **`let session = db.getMongo().startSession();`**:  Starts a new session.

3. **`session.startTransaction();`**: Begins a transaction within the session.

4. **`db.collection1.insertOne(...)` and `db.collection2.updateOne(...)`**: These are your database operations (insert and update in this example).  **Crucially, the `session: session` option is passed to each operation.** This tells MongoDB that these operations are part of the specified transaction.

5. **`print("Insert Result 1:", result1);` and `print("Update Result 2:", result2);`**: For operations that modify data within a transaction on a sharded cluster, you will see the `$txnNumber` and `lsid` in the output.  These confirm that the operation was executed as part of the transaction.

6. **`session.commitTransaction();`**:  Commits the transaction.  All the operations become permanent.

7. **`session.abortTransaction();`**:  Aborts the transaction if any error occurs.  All changes are rolled back, ensuring atomicity.

8. **`session.endSession();`**:  Ends the session.  It's important to close the session to release resources.  This is done in the `finally` block to ensure it's always executed, even if errors occur.


* **Sharded Clusters:** Transactions are most beneficial in sharded clusters where data is distributed across multiple shards.  The `$txnNumber` and `lsid` are particularly important in this scenario.  Transactions on a single standalone server or a replica set are still supported, but their primary value is when coordinating changes across shards.
* **Replica Sets:** Transactions can also be useful in replica sets, where they can help ensure data consistency if a primary failover occurs during a multi-document operation.
...
- La taille des documents impact négativement la performance des requetes. Il faut faire attention a ne pas tout mettre dans un document, mais restreindre aux informations pertinentes. Limiter la taille des documents permet d'améliorer la performance des requetes.
...
- renverser l'integration et inserer le nom du `server` dans chaque document `event`! duplication des données mais rapidité de requetage et pas de risque de dépasser la taille de 16Mb
