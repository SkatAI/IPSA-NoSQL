...
Dans ce projet vous allez creer une nouvelle base de données MongoDB a partir d'un dataset de votre choix.

Vous pouvez travailler en solo ou par groupes de 2.

Tout au long du projet vous ecrirez un journal de bord de votre experience.

- justifier le choix du dataset
- décrire l'application qui va l'exploiter
- contexte matériel,  outils utilisés
- bugs, problèmes etc
- performance

Le journal de bord doit être un document word. Invitez moi en tant qu'editeur alexis.perrier@gmail.com

## Choix du dataset

Le choix du dataset est libre. Seule condition, il doit etre assez grand, plusieurs Gb de données

- Kaggle datasets
- Nasa, ESA, ... datasets
- open data paris, gov.fr etc
- huggingface

Si votre dataset est principaleent composé d'images, sons, vidéos, etc , il faut trouver une solution de stockage cloud.
Ne pas stocker les images directemnt dans les documents MongoDB.

Le dataset peut comporter des textes longs (articles, livres, ...)
...
Par exemple, données de arbres de Paris:

- application de terrain de mise a jour des dimensions des arbres
- application d'exploration des arbres de paris pour botanistes ou touristes
- application de gestion des arbres dédiée a la selection des types d'arbres et la geolocalisation

Quand vous avez une idée de votre application, ecrivez les requetes de haut niveau,
les operations qui selon vous seront les plus fréquentes.
...
Documentez-les soigneusement, car elles constituent des sources précieuses d'enseignement pour vous et vos camarades.
