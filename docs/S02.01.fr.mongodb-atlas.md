...
Dans ce document nous allons travailler sur MongoDB Atlas, un service hébergé de MongoDB pour créer une base de données, charger des documents et executer des requêtes.

Dans un second temps nous verrons comment travailler sur une base de données plus conséquente en Python ou dans le terminal avec `mongosh`.
...
2 documents, c'est méga excitant 🤣🤣🤣 !
...
[TODO] comment trouver la string de connection
...
Une fois connecté a une instance
...
#### En python
Exécutons quelques requêtes en Python sur la base de données `movies`
